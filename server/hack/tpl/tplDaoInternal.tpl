...
func (dao *{TplTableNameCamelCase}Dao) DB() gdb.DB {
	return dbutil.DB(dao.group).GDB()
}
...
func (dao *{TplTableNameCamelCase}Dao) Model(ctx context.Context) *gdb.Model {
	return dao.DB().Model(dao.table).Safe().Ctx(ctx)
}
// Ctx creates and returns the Model for current DAO, It automatically sets the context for current operation.
func (dao *{TplTableNameCamelCase}Dao) Ctx(ctx context.Context) *gdb.Model {
	return dao.DB().Model(dao.table).Hook(dbutil.CacheHook(dao.group,time.Hour)).Safe().Ctx(ctx)
}
